...
$payload = @{
    ...
    parties = @(
        @{
            nomComplet = "Test Dem"
            typePersonne = "Physique"  # Pascal
            typePartie = "Demandeur"   # Pascal
            qualite = "Salarié"
            adresse = "Test Adr"
             nomEntite = ""
            numeroRccm = ""
...
try {
    ...
    Write-Host "PascalCase Succeeded (Unexpected)"
    $response | ConvertTo-Json
} catch {
    Write-Host "PascalCase Failed (Expected)"
